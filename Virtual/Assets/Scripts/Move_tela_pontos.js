...
static  var colidiu_porta			: boolean;
...
function Update()
{
	...
	if (moveX)
	{
		...
		else if (status == "MovingToEnd")
		{
			...
			transformToMove.transform.localPosition.x = Mathf.Lerp(startPos, endPos, (currentTime / 60) * currentSpeed);
		}
		else if (status == "MovingToStart")
		{
			currentTime++;
			currentSpeed = Mathf.Lerp(startSpeed, endSpeed, (currentTime / 60));
			transformToMove.transform.localPosition.x = Mathf.Lerp(endPos, startPos, (currentTime / 60) * currentSpeed);
		}
	}
	...
	if (moveY)
	{
		...
		else if (status == "MovingToEnd")
		{
			currentTime++;
			currentSpeed = Mathf.Lerp(startSpeed, endSpeed, (currentTime / 60));
			transformToMove.transform.localPosition.y = Mathf.Lerp(startPos, endPos, (currentTime / 60) * currentSpeed);
		}
		...
		else if (status == "MovingToStart")
		{
			currentTime++;
			currentSpeed = Mathf.Lerp(startSpeed, endSpeed, (currentTime / 60));
			transformToMove.transform.localPosition.y = Mathf.Lerp(endPos, startPos, (currentTime / 60) * currentSpeed);
		}
	}
}
